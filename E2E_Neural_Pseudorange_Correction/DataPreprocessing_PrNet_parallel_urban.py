...
def data_preprocessing(enc_x, device):
    ...
    # 7, 8, 9. Ground truth of longitude
    post_enc_x_seq.append(enc_x[:, :, 39:42]/torch.tensor([180, 60, 60]).to(device))

    # 10, 11, 12. Ground truth of latitude
    post_enc_x_seq.append(enc_x[:, :, 42:45]/torch.tensor([90, 60, 60]).to(device))
