...
def data_preprocessing(enc_x, device):
    ...
    # 7, 8, 9. EKF based longitude
    post_enc_x_seq.append(enc_x[:, :, 45:48]/torch.tensor([180, 60, 60]).to(device))

    # 10, 11, 12. EKF based latitude
    post_enc_x_seq.append(enc_x[:, :, 48:51]/torch.tensor([90, 60, 60]).to(device))
